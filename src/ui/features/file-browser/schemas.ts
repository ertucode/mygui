...
export function resolveSort(sort: SortContext): SortState {
  ...
  if (
    (sort.directory && sort.directory.basedOn === localOrGlobal) ||
    localOrGlobal == null
  ) {
...
export function resolveSortFromStores(
  dir: DirectoryContextDirectory,
  columnPrefs: ColumnPreferenceStore,
) {
  return resolveSort({
