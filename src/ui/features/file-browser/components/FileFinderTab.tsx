...
export function FileFinderTab({ isOpen, onClose, showPreview }: FileFinderTabProps) {
  ...
  useEffect(() => {
    ...
    const searchFiles = async () => {
      ...
      try {
        ...
        const result = await getWindowElectron().fuzzyFileFinder(directory.fullPath, query)
        if (result.success) {
          setFilteredFiles(result.data)
          setSelectedIndex(0)
        } else {
          setError(errorResponseToMessage(result.error) || 'Failed to load files')
        }
      } catch (err) {
  ...
  const selectedFilePath = selectedFile ? directoryHelpers.getFullPath(selectedFile, activeDirectoryId) : null
  const selectedFileExt = selectedFile ? PathHelpers.getDottedExtension(selectedFile) : null
    ...
    <div className="flex gap-3 h-full overflow-visible">
      <div className="flex flex-col gap-3 flex-1 min-w-0 h-full">
        ...
        <div className="flex-1 min-h-0 border border-gray-200 rounded overflow-hidden flex flex-col">
          {isLoading && <div className="text-center text-gray-500 py-4">Loading files...</div>}
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex gap-3 items-center min-w-0 flex-1">
                          <span className="text-xs truncate">{highlightMatch(fileName, query)}</span>
                          {folder && <span className="text-xs text-gray-500 truncate">{folder}</span>}
                        </div>
                      </div>
                    )
                  })}
        ...
        <div className="text-xs text-gray-500 flex flex-wrap gap-x-4 gap-y-1 justify-center flex-shrink-0">
          <div>
            <kbd className="kbd">↑↓</kbd> or <kbd className="kbd">Ctrl+J/K</kbd> to navigate
          </div>
          <div>
