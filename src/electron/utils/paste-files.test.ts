...
import os from 'os'
import { pasteFiles as pf } from './paste-files.js'
import { copyFiles, clearClipboardState } from './copy-files.js'

const pasteFiles = async (...args: Parameters<typeof pf>) => {
  const res = await pf(...args)
  if ('customPaste' in res) {
    throw new Error('customPaste not supported in test')
  }
  return res
}
...
class TestEnvironment {
  ...
  async setup() {
    ...
    this.tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'koda-paste-test-'))
  ...
  async createFile(dirPath: string, fileName: string, content: string = 'test content'): Promise<string> {
  ...
  async createNestedStructure(baseDir: string, structure: Record<string, string | Record<string, any>>): Promise<void> {
...
describe('paste-files.ts', () => {
  ...
  describe('No conflicts - simple paste', () => {
    test('should paste a single file without conflicts', async () => {
      // Setup
      const sourceFile = await env.createFile(env.sourceDir, 'test.txt', 'hello')
  ...
  describe('Conflict detection', () => {
    test('should detect file conflict', async () => {
      ...
      const sourceFile = await env.createFile(env.sourceDir, 'conflict.txt', 'source version')
    ...
    test('should generate unique suggested names', async () => {
      ...
      const sourceFile = await env.createFile(env.sourceDir, 'test.txt', 'original')
  ...
  describe('Conflict resolution - global strategies', () => {
    test('should override files with global override strategy', async () => {
      // Setup
      const sourceFile = await env.createFile(env.sourceDir, 'file.txt', 'new content')
  ...
  describe('Conflict resolution - per-file overrides', () => {
    test('should apply custom name to specific file', async () => {
      // Setup
      const sourceFile = await env.createFile(env.sourceDir, 'original.txt', 'content')
  ...
  describe('Cut operations', () => {
    test('should move file with cut=true and no conflicts', async () => {
      // Setup
      const sourceFile = await env.createFile(env.sourceDir, 'move-me.txt', 'content')
    ...
    test('should prevent cut and paste in same directory', async () => {
      ...
      if (!result.needsResolution) {
        ...
        if (!result.result.success && result.result.error.type === 'message') {
          expect(result.result.error.message.includes('Cannot cut and paste')).toBeTruthy()
        }
      }
    })
  })
  ...
  describe('Edge cases and error handling', () => {
    ...
    test('should reject invalid custom names', async () => {
      // Setup
      const sourceFile = await env.createFile(env.sourceDir, 'file.txt', 'content')
  ...
  describe('Comprehensive validation', () => {
    test('should reject invalid global strategy', async () => {
      ...
      if (!result.needsResolution) {
        expect(result.result.success).toBe(false)
        if (!result.result.success && result.result.error.type === 'message') {
          expect(result.result.error.message.includes('Invalid global strategy')).toBeTruthy()
        }
      }
    })
    ...
    test('should reject customName action without customName value', async () => {
      ...
      if (!result.needsResolution) {
        expect(result.result.success).toBe(false)
        if (!result.result.success && result.result.error.type === 'message') {
          expect(result.result.error.message.includes('Custom name required')).toBeTruthy()
        }
      }
    })
    ...
    test('should reject duplicate destination paths from custom names', async () => {
      ...
      if (!result.needsResolution) {
        expect(result.result.success).toBe(false)
        if (!result.result.success && result.result.error.type === 'message') {
          expect(result.result.error.message.includes('Multiple files would be pasted')).toBeTruthy()
        }
      }
    })
    ...
    test('should validate per-file override that prevents all pastes', async () => {
      ...
      if (!result.needsResolution) {
        expect(result.result.success).toBe(false)
        if (!result.result.success && result.result.error.type === 'message') {
          expect(result.result.error.message.includes('No files would be pasted')).toBeTruthy()
        }
      }
    })
  })
