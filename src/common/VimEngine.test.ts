...
function expectStateEqual(
  actual: VimEngine.State,
  expected: Partial<VimEngine.State>,
) {
  if (expected.cursor) {
...
interface TestCase {
  ...
  operation: (state: VimEngine.State) => VimEngine.State;
  expected: Partial<VimEngine.State>;
}
...
describe("VimEngine - Multi-operation workflows", () => {
  it("yy, p, dd, u should restore to state after paste", () => {
    ...
    expect(state.currentBuffer.items.map((i) => i.str)).toEqual([
      "line 1",
      "line 2",
      "line 2",
      "line 3",
  ...
  it("cc, type new line (simulated), u should restore original line", () => {
    ...
    state = {
      ...
      currentBuffer: {
        ...state.currentBuffer,
        items: [
          { type: "str", str: "new line" },
          ...state.currentBuffer.items.slice(1),
        ],
      },
      mode: "normal",
    };
  ...
  it("P at start of file, then multiple undos", () => {
    let state = createState(["line 1", "line 2"], 0, 0, "normal", 0, [
      "new line",
    ]);
