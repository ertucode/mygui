...
export function FolderFinderTab({
    ...
    const searchFolders = async () => {
      ...
      try {
        ...
        if (result.success) {
          ...
          setError(
            errorResponseToMessage(result.error) || "Failed to load folders",
          );
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const loadContents = async () => {
      ...
      try {
        const fullPath = directoryHelpers.getFullPath(
          selectedFolder,
          activeDirectoryId,
        );
        const contents =
  ...
  const handleSelect = (folderPath: string) => {
    directoryHelpers.cdFull(
      directoryHelpers.getFullPath(folderPath, activeDirectoryId),
      activeDirectoryId,
    );
    onClose();
  };
  ...
  const selectedFolderName = selectedFolder
    ? getFolderNameAndParent(selectedFolder).folderName
    : null;
    ...
    <div className="flex gap-3 h-full overflow-visible">
      <div className="flex flex-col gap-3 flex-1 min-w-0 h-full">
                      ...
                      >
                        ...
                        <div className="flex gap-3 items-center min-w-0 flex-1">
                          <span className="text-xs truncate">
                            {highlightMatch(folderName, query)}
                          </span>
                          {parent && (
        ...
        <div className="text-xs text-gray-500 flex flex-wrap gap-x-4 gap-y-1 justify-center flex-shrink-0">
          <div>
            <kbd className="kbd">Up/Down</kbd> or{" "}
