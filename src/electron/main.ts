...
app.on('ready', () => {
  ...
  ipcHandle('getApplicationsForFile', filePath => getApplicationsForFile(filePath))
  ipcHandle('openFileWithApplication', async ({ filePath, applicationPath }) => {
  ...
  const selectAppPromises = new Map<number, (appPath: string | null | undefined) => void>()
  ...
  ipcMain.on('selectAppWindowResult', (event: Electron.IpcMainEvent, appPath: string | null | undefined) => {
    const window = BrowserWindow.fromWebContents(event.sender)
    if (window) {
      const resolver = selectAppPromises.get(window.id)
      if (resolver) {
        if (appPath) {
          if (appPath.endsWith('.app') && appPath.startsWith('/Applications')) {
