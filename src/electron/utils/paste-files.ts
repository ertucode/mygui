...
async function copyRecursive(src: string, dest: string, onFileCopied?: () => void) {
  ...
  if (stats.isDirectory()) {
    ...
    for (const entry of entries) {
      await copyRecursive(path.join(src, entry), path.join(dest, entry), onFileCopied)
    }
  } else {
    // Copy file
...
async function generateUniqueName(destinationDir: string, fileName: string): Promise<string> {
  ...
  try {
    ...
    try {
      ...
      if (conflicts.length < limit) {
        const suggestedName = await generateUniqueName(path.dirname(destPath), path.basename(destPath))
        ...
        conflicts.push({
          ...
          sourceSize,
          destSize,
          sourceSizeStr: sourceStats.isDirectory() ? '--' : formatFileSize(sourceSize),
          destSizeStr: destStats.isDirectory() ? '--' : formatFileSize(destSize),
        })
      }
    } catch {
...
async function findConflicts(
  ...
  for (const sourcePath of sourcePaths) {
    await findConflictsRecursive(sourcePath, destinationDir, path.basename(sourcePath), conflicts, limit, totalCount)
  }
...
async function validateResolution(
  ...
  if (!resolution.globalStrategy || !['override', 'trash', 'autoName', 'skip'].includes(resolution.globalStrategy)) {
    return {
      valid: false,
  ...
  if (resolution.perFileOverrides) {
    for (const [destPath, override] of Object.entries(resolution.perFileOverrides)) {
      ...
      if (!['override', 'trash', 'customName', 'skip'].includes(override.action)) {
        return {
          valid: false,
  ...
  for (const sourcePath of filePaths) {
    ...
    let finalAction: 'override' | 'trash' | 'autoName' | 'skip' = resolution.globalStrategy
  ...
  if (finalDestinations.size === 0) {
    return {
      valid: false,
      error: 'No files would be pasted with this resolution (all files skipped)',
    }
  }
...
async function executePasteWithResolutions(
  ...
  try {
    ...
    const validation = await validateResolution(filePaths, destinationDir, resolution)
    if (!validation.valid) {
  ...
  try {
    ...
    if (isCut) {
      ...
      if (allInSameDir) {
        return {
          needsResolution: false,
          result: GenericError.Message('Cannot cut and paste in the same directory'),
        }
      }
    }
    ...
    if (!resolution) {
      ...
      const totalFiles = (await Promise.all(filePaths.map(countFilesInPath))).reduce((a, b) => a + b, 0)
      ...
      const abortSignal = taskId ? TaskManager.getAbortSignal(taskId) : undefined
      ...
      for (let i = 0; i < filePaths.length; i++) {
        ...
        if (abortSignal?.aborted) {
          ...
          const result =
            pastedItems.length > 0 ? Result.Success({ pastedItems }) : GenericError.Message('Operation cancelled')
    ...
    try {
      ...
      TaskManager.result(
        taskId,
        error instanceof Error ? GenericError.Message(error.message) : GenericError.Unknown(error)
      )
      throw error
    }
  } catch (error) {
